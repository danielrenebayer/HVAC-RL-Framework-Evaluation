...
checkpoint_dir=$(realpath "../checkpoints/s303-npar")"/${datestr}"
num_iters=100
...
for i in $(seq $num_iters); do
    ...
    arguments+=( "--reward_function" "sum_emean_ediff_mstpc" )
    arguments+=( "--lambda_rwd_energy" 0.07 )
    arguments+=( "--lambda_rwd_mstpc"  0.12 )
    arguments+=( "--clip_econs_at" 35.0 )
