...
checkpoint_dir=$(realpath "../checkpoints/s308")"/${datestr}"
num_iters=100
num_iters_post=20 #after epsilon set to 0.5
num_episodes_per_iter=160

let num_total_iters=$num_iters*$num_iters_post
...
let num_iters_post_1=$num_iters
let num_iters_post_2=$num_iters+$num_iters_post/2
mkdir -p $checkpoint_dir

for i in $(seq $num_total_iters); do
    arguments=()
    ...
    if   (( $i < $num_iters_quart )); then
        ...
        arguments+=( "--lr" 0.04 )
    elif (( $i < $num_iters_post_1 )); then
        arguments+=( "--lr" 0.02 )
    elif (( $i < $num_iters_post_2 )); then
        arguments+=( "--lr" 0.01 )
    else
        arguments+=( "--lr" 0.005 )
    fi
    ...
    arguments+=( "--lambda_rwd_mstpc"  0.08 )
